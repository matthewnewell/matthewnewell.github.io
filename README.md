# Matthew Newell
## Senior Principal Engineer | AI Orchestration
I focus on building **Semi-Disposable Software** and **Applied AI solutions** that solve real-world technical debt. Skeptical of enterprise initiatives.

---

## 🚀 Key Areas of Expertise
- **Systems Integration:** 20+ years managing multi-million dollar defense & aerospace programs (V2X, RTX, Northrop Grumman, TRW).
- **Applied AI:** Developing autonomous agents and RAG pipelines using Antigravity and Gemini.
- **Project Leadership:** PMP certified, specializing in Weapons System Integration and augmenting business process workflows with AI.

## 🛠 Currently Building
- **Universal Growth Engine:** A personal "Life Operating System" built on Google Drive & Gemini.
- **Musical Mastery:** Applying "Atomic Habits" to music theory and performance.
- **[Burned Value](https://github.com/matthewnewell/BurnedValue):** The lovechild of EVMS and a Release Burndown: the fiscal discipline of a government auditor meets the 'pivot-or-perish' soul of a startup.
- **[CMMI 1.3-AI: Automated Process Orchestration](https://github.com/matthewnewell/CMMI-1.3-AI-Orchestration)** Develop specialized AI Agents for CMMI Process Areas.

---
[LinkedIn](https://www.linkedin.com/in/matthew-newell-pmp-9784746/) | [Email](mailto:matthew.r.newell@gmail.com)